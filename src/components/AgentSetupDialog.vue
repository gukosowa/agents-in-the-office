...
function buildClaudeScriptContent(): string {
  return [
    ...
    '',
    'TMP_FILE=$(mktemp "$EVENTS_DIR/.tmp.XXXXXX")',
    'jq -n \\',
    ...
    '  rm -f "$SESSION_FILE"',
    'else',
    '  TMP_SESSION=$(mktemp "$SESSIONS_DIR/.tmp.XXXXXX")',
    '  jq -n \\',
    '    --arg sessionId "$SESSION_ID" \\',
    '    --argjson timestamp "$TS" \\',
    ...
    '      agentType: "claude_code",',
    '      payload: $payload',
    "    }' > \"$TMP_SESSION\" && mv \"$TMP_SESSION\" \"$SESSION_FILE\"",
    'fi',
    '',
  ].join('\n');
}
...
function buildGeminiScriptContent(): string {
  return [
    ...
    "  } | with_entries(select(.value != null))')",
    '',
    '# Write event file atomically and output empty JSON to stdout (Gemini requires JSON)',
    'TMP_FILE=$(mktemp "$EVENTS_DIR/.tmp.XXXXXX")',
    'jq -n \\',
    '  --arg sessionId "$SESSION_ID" \\',
    '  --argjson timestamp "$TS" \\',
